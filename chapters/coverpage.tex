...
%输入论文题目如果过长可以在第二行进行输入，如果不需要第二行请自行删除，注意字体里面的\xiaoerd表示的是单倍行距，在封面页各种字体后面都加一个d
			...
			\makecell[c]{\songti\bfseries\xiaoerd 论文题目}	& \makecell[c]{\songti\bfseries\xiaoerd 基于SAR引导的光学图像} \\ 
			\cline{2-2}
			 	&  \makecell[c]{\songti\bfseries\xiaoerd 去云方法研究}\\ 
			\cline{2-2}
			\end{tabular}
	\end{table}
...
%这是一个表格，请在相应位置输入相关信息 
       \begin{tabular}{p{2.5cm}p{9.4cm}}		
		...
		\cline{2-2} 
		\makecell[c]{\songti\bfseries\sanhaod 学\qquad 号} 	&  \makecell[c]{\songti\bfseries\sanhaod S230201040} \\
		\cline{2-2} 
		\makecell[c]{\songti\bfseries\sanhaod 作者姓名} 	& \makecell[c]{\songti\bfseries\sanhaod 胡美琳} \\
		\cline{2-2} 
		\makecell[c]{\songti\bfseries\sanhaod 指导教师} 	& \makecell[c]{\songti\bfseries\sanhaod 罗小波} \\
		\cline{2-2} 
		\makecell[c]{\songti\bfseries\sanhaod 学\qquad 院} 	&  \makecell[c]{\songti\bfseries\sanhaod 计算机学院} \\
		\cline{2-2}
...
%这也是一个表格，请在相关位置输入相关信息		
		...
		\makecell[l]{\songti\xiaosid 分\hspace{6pt}类\hspace{6pt}号} 	&\makecell[c]{\xiaosid xxxxxx} &\makecell[c]{\songti\xiaosid 密级} & \makecell[c]{\xiaosid 公开} \\
		\cline{2-2} \cline{4-4}
...
\dlmu[14cm]{\songti\bfseries\xiaoerd 基于SAR引导的光学图像去云方法研究}\\
\vspace{5mm}
%请在此输入作者姓名
\dlmu[5cm]{\songti\bfseries\sanhaod 胡 \quad 美 \quad 琳}\\
...
%请在相关位置输入指导教师姓名，如果有第二导师请在第二行输入，如果没有，可以自行删去第二行
		\makecell[r]{\songti\xiaosid 指导教师}	& \makecell[c]{\songti\bfseries\sanhaod  \quad \, 罗小波 \quad \, \qquad 教\hspace{16pt}授} \\
		\cline{2-2}
	\end{tabular}
\end{table}
...
%请在相关位置输入相关信息，工程硕士、学硕及博士的部分类容有不同，请自行修改	
	...
	\makecell[l]{\songti\xiaosid 专业学位领域} 	& \multicolumn{5}{c}{\songti\bfseries\sihaod 计算机视觉} \\
	 \cline{2-6}
...
%	\begin{spacing}{1.0}
%英文论文题目自行修改
	\xiaoerd\textbf{Research on SAR-guided optical image cloud removal methods}\\
	
	\vspace{60mm}
%根据硕士或者博士，自行修改下面语句中的论文种类，标准请参照写作指南	
	\xiaosand A Master Dissertation Submitted to \\
...
%这是一个表格，在相应位置输入相关信息		
		...
		\cline{2-2} 
		\makecell[r]{\sanhaod Student ID} 	&  \makecell[c]{\bfseries\sanhaod S230201040} \\
		\cline{2-2} 
		\makecell[r]{\sanhaod Author} 	& \makecell[c]{\bfseries\sanhaod HU meilin} \\
		\cline{2-2} 
		\makecell[r]{\sanhaod Supervisor} 	& \makecell[c]{\bfseries\sanhaod Prof. LUO Xiaobo} \\
		\cline{2-2} 
