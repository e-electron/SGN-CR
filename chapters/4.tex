...
在原 SGN-CR 中，光学分支在三个尺度层级，分别堆叠 8 个 Opt-block，并采用 $C$， $2C$， $4C$ 的通道扩展方式进行特征建模。 而在 Lite-SGN-CR 中将保留的三级下采样结构中的的每一尺度的模块堆叠次数由 8 减少为 4，并将三个阶段的输出通道分别由原网络的配置缩减至 todo通道：48、96、192，从而在维持基本多尺度表征能力的同时显著降低注意力相关计算的总体开销。
...
从表~\ref{tab:Lite-SGN-CR-efficiency} 可以看出，Lite-SGN-CR 的参数规模为 5.81M，仅为 SGN-CR 的 37.8\%，相比 GLF-CR 与 USSRN-CR 等模型亦明显更小。在理论计算量方面，Lite-SGN-CR 的 FLOPs 为 16.58G，相较于 GLF-CR（61.39G）与 USSRN-CR（305.80G）大幅下降，在当前对比模型中处于较低水平。
...
\begin{figure*}[htbp]
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} ccccc @{}}
		...
		\incC{width=0.18\textwidth}{chapters/figures/figure/b_SAR-Opt-cGAN.png} \\[-3ex]
		...
		\incC{width=0.18\textwidth}{chapters/figures/figure/b_SGN-CR(Ours).png} \\[-3ex]
		...
		\incD{width=0.18\textwidth}{chapters/compare_img/sar-opt-cgan/ROIs1158_spring_123_p46.png} \\[-3ex]
		\makecell[c]{\wuhao SAR} &
		\makecell[c]{\wuhao Cloudy} &
		\makecell[c]{\wuhao Cloud-Free} &
		\makecell[c]{\wuhao GANs} &
		...
		\incD{width=0.18\textwidth}{chapters/compare_img/ours/ROIs1158_spring_123_p46.png} \\[-3ex]
		\makecell[c]{\wuhao GLF-CR} &
		\makecell[c]{\wuhao USSRN-CR} &
		\makecell[c]{\wuhao GCEPANet} &
		\makecell[c]{\wuhao SGN-CR} &
...
\begin{table}[!htbp]
	...
	\begin{tabular}{@{}>{\songti\wuhao}p{0.20\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.20\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.20\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.20\textwidth}@{}}
		...
		\hline
		Params(M)& 15.36 & 5.81 & \textbf{-9.55} \\
		FLOPs(G)& 31.02 & 16.58 & \textbf{-14.44} \\
...
结果表明，在 PSNR 仅下降 0.1134 dB、SSIM 仅下降 0.0026 的情况下，Lite-SGN-CR 的参数量与 FLOPs 分别降低 62.2\% 与 46.6\%，推理延迟亦下降约 53.2\%。说明所提出的轻量化设计主要压缩了结构中的冗余计算路径，而未破坏跨模态结构引导机制的核心表达能力。
...
\begin{figure}[h]
	...
	% SGN-CR (black)
		\addplot[only marks, mark=*, mark size=3.2pt, black] coordinates {(31.02,30.5503)};
	...
	\addlegendentry{Lite-SGN-CR}
		...
		% Arrow only
		\draw[->, thick]
			(axis cs:31.02,30.5503) -- (axis cs:16.58,30.4369);
...
\hline
C1: [64,128,256] & 7.90 & 36.0 & 30.58 & 0.8994 \\
C2: [48,96,192] & todo6.60 & 30.0 & 30.55 & 0.8990 \\
\textbf{C3: [32,64,128]} & \textbf{5.81} & \textbf{16.58} & \textbf{30.4369} & \textbf{0.8964} \\
C4: [24,48,96] & todo4.10 & 16.0 & 30.20 & 0.8935 \\
C4: [16,32,64] & todo4.10 & 16.0 & 30.20 & 0.8935 \\
\bottomrule[1.5pt]
\end{tabular}
\end{table}
...
\begin{table}[!htbp]
	...
	\begin{tabular}{@{}>{\arraybackslash\songti\wuhao}p{0.24\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.14\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.14\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.14\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.14\textwidth}@{}}
		...
		\hline
		\makecell[l]{原始 SAR 编码器 \\ (64-128-256)} & todo6.40 & 28.5 & 30.56 & 0.8992 \\
		\textbf{\makecell[l]{Lite SAR 编码器 \\ (32-64-128)}} & \textbf{5.81} & \textbf{16.58} & \textbf{30.4369} & \textbf{0.8964} \\
		\makecell[l]{过度压缩 SAR 编码器 \\ (16-32-64)} & todo4.85 & 21.0 & 30.20 & 0.8940 \\
		\bottomrule[1.5pt]
	\end{tabular}
\end{table}
...
\hline
P0: SGN-CR & 15.36 & 31.02 & 30.5503 & 0.8990 \\
P1: P0 + Lite-SAR & todo9.60 & 44.0 & 30.57 & 0.8992 \\
P2: P1 + Optical Width@C2 & todo8.20 & 36.0 & 30.56 & 0.8990 \\
P3: P1 + Optical Width@C3 & todo6.80 & 28.0 & 30.52 & 0.8986 \\
P4: P3 + Lite-CMCA & todo5.81 & 16.58 & 30.4369 & 0.8964 \\
...
\begin{table}[!htbp]
	...
	\begin{tabular}{@{}>{\centering\arraybackslash\songti\wuhao}p{0.14\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.13\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.12\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.12\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.12\textwidth}>{\centering\arraybackslash\songti\wuhao}p{0.13\textwidth}@{}}
		...
		\hline
		1 & 5.81 & todo18.6 & 42.3 & 29.84 & 0.8912 \\
		2 & 5.81 & todo37.2 & 84.5 & 30.42 & 0.8968 \\
		3 & 5.81 & 16.58 & 9.84 & 30.4369 & 0.8964 \\
		4 & 5.81 & todo74.4 & 169.4 & 30.75 & 0.8998 \\
		5 & 5.81 & todo92.0 & 239.1 & 30.34 & 0.8998 \\
		\bottomrule[1.5pt]
	\end{tabular}
\end{table}
...
\begin{figure*}[htbp]
	...
	\label{fig:sstage_visualization}
	\renewcommand{\arraystretch}{1.2}
	\wuhao
		...
		\includegraphics[width=0.14\textwidth]{chapters/compare_img/ours/ROIs2017_winter_108_p51.png} &
		\includegraphics[width=0.14\textwidth]{chapters/compare_img/hpn/ROIs2017_winter_108_p51.png} \\[0.2ex]
...
在相同迭代次数条件下，引入门控机制后 PSNR 提升约 0.14 dB，SSIM 同步提升，MAE 略有下降。无门控递归在多轮更新过程中对全图区域进行均匀修正，易在已恢复区域引入累积扰动；而门控残差机制通过逐像素调节更新强度，使修正更加集中于高不确定区域，从而提升了递归细化的稳定性与最终重建质量。
